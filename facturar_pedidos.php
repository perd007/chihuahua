...
///////			
	...
	$insertSQL = sprintf("INSERT INTO facturas ( id_cliente, fecha, serie, numero, sub_total, iva, total, transaccion, efectivo, debito, cheque, retencion) VALUES (%s, NOW(), %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
  ...
  if($Result2==1){
	  //actualizamos el almacen
		$eliminar = sprintf("INSERT INTO almacen (id_producto, factura_venta, serie_venta, cantidad, transaccion) VALUES (%s, %s, %s, %s, %s)",
...
//validamos si fue exitosa la carga para eliminar los pedidos e insertar venta en el almacen	
	if($Result1==1 and $totalResult>=1 and $totaleliminar>=1){
		...
		$row_clientes = mysql_fetch_array($clientes);
		 ...
		 echo "<script type=\"text/javascript\">alert ('FACTURADO');   </script>";	
		 echo "<script type=\"text/javascript\">alert ('EN ESPERA QUE SE CULMINE LA IMPRESION POR MAQUINA FISCAL');  location.href='fondo.php' </script>";
		}
	}
//
    ...
    <tr>
      ...
      <td colspan="3" valign="top" class="tituloDOSE_2">Tipo de Pago
        ...
        <label for="radio5">Efectivo 
          <input name="efectivo" type="text" id="efectivo" value="<?php echo $row_pedido['total']; ?>" size="11"  disabled="disabled" maxlength="11" />
...
<table border="0" cellpadding="0" cellspacing="0" width="775">
  <tbody>
    ...
    <tr >
      <td valign="top">
      <table width="739" align="center" class="tblLista">
        <thead>
          <tr >
       <th width="80" scope="col">Codigo</th>
		...
		?>
          <tr id="lista_detalles">
            <td width="80" align="center" scope="col"><?php echo $row_productos['codigo']; ?>
              <input type="hidden" name="codigo<?=$i?>" value="<?=$row_productos['codigo']?>" />
              <input type="hidden" name="id_prod<?=$i?>" value="<?=$row_productos['id_producto']?>" />
            </td>
            <td width="212" align="center" scope="col"><?php echo $row_productos['nombre']; ?></td>
            ...
            <td width="86" align="center" scope="col"><?php echo $row_productos_pedidos['precio']; ?>  
            <input type="hidden" name="precio<?=$i?>" id="precio<?=$i?>" value="<?=$row_productos_pedidos['precio']?>" />
            </td>
            <td width="86" align="center" scope="col"><?php echo $total; ?>
            </td>
          </tr>
          <?php 
		  $i++;

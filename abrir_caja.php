...
if($_COOKIE["val"]==true){
	if($_COOKIE["ac"]!=1){
	echo "<script type=\"text/javascript\">alert ('Usted no posee permisos para Abrir Caja');location.href='fondo.php' </script>";
    exit;
	}
}
else{
...
if($totalRows_cajas>=1){
	echo "<script type=\"text/javascript\">alert ('Ya la Caja esta Abierta'); location.href='fondo.php' </script>";
 exit;
}
...
if($row_permisos["ac"]==1){
	
  $insertSQL = sprintf("INSERT INTO caja (fecha_ap,  estado) VALUES (NOW(), %s)",
                       GetSQLValueString("ABIERTA", "text"));

  mysql_select_db($database_conexion, $conexion);
  $Result1 = mysql_query($insertSQL, $conexion) or die(mysql_error());
   if($Result1){
	 echo "<script type=\"text/javascript\">alert ('Caja Abierta'); location.href='fondo.php' </script>";
	}//fin del if
	
}//fin del if
else{
	echo "<script type=\"text/javascript\">alert ('Usted no Posee perminsos para Abrir Cajas'); location.href='fondo.php' </script>";
 exit;
}//fin del else
...
function validar(){
			...
			var valor=confirm('¿Esta seguro de Apeturar Caja?');
			if(valor==false){
			return false;
			}
			else{
  ...
  <table width="559" border="0" align="center" cellpadding="0" cellspacing="0">
    <tbody>
      <tr>
        ...
        <td align="center" valign="center" background="imagenes/v_back_sup.jpg" class="tituloDOSE_3"><span class="negrita">Apertura de caja</span></td>

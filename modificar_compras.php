				...
				$i=1;
				 ...
				 while($_POST["nro_detalles"]>=$i){
						...
						if($eli_prod==1 and $eli_prod2==0){
							...
							$query_almacen = "SELECT sum(cantidad) FROM almacen where id_producto='$field_cp[producto]' and (transaccion='COMPRA' or transaccion='COMPRA-MODIFICADA') ";
							$almacen = mysql_query($query_almacen, $conexion) or die(mysql_error());
							$row_almacen = mysql_fetch_assoc($almacen);
							
							$query_almacen2 = "SELECT sum(cantidad) FROM almacen where  id_producto='$field_cp[producto]' and transaccion='VENTA' ";
							...
							$query_almacen3 = "SELECT sum(cantidad) FROM almacen where  id_producto='$field_cp[producto]' and transaccion='EXTRAIDO' ";
							...
							$disponible=$row_almacen["sum(cantidad)"]-$row_almacen2["sum(cantidad)"]-$row_pedido["sum(cantidad)"]-$row_almacen3["sum(cantidad)"]-$row_apartado["sum(cantidad)"];
  ...
  //modificamos los productos
	 ...
	 while($_POST["nro_detalles"]>=$i){ 
		...
		$num = str_replace(",",".",$_POST['costo'.$i]);
				...
				//cargamos el almacen
				$insertSQLA = sprintf("INSERT INTO almacen (id_producto, factura_compra, cantidad, transaccion) VALUES (%s, %s, %s, %s)",
...
//actualizamos el almacen
	$insertSQL3 = sprintf("UPDATE almacen SET factura_compra=%s, cantidad=%s, transaccion=%s where id_producto=%s and factura_compra=%s and transaccion!='ELIMINADO POR MODIFICACION'",
...
//actualizamos datos basicos
  ...
  $updateSQL = sprintf("UPDATE compras SET fecha=%s, num_fac=%s, proveedor=%s, fecha_limite=%s, fiador=%s WHERE id_compras=%s",
...
/////////////////////////////////////////////////////////////////////////////
 ...
 //---------------------
  ...
  echo "<script type=\"text/javascript\">alert ('ACTUALIZACION EXITOSA');  location.href='' </script>";
  }else{
...
var costo=document.getElementById("costo" + nro).value.split(',').join('.' );
var total=cantidad * costo;
document.getElementById("total" + nro).value = redondeo2decimales(total);
      ...
      <tr>
        <td width="759"><table border="0" cellpadding="0" cellspacing="0" width="742">
          <tbody>
                      ...
                      <tbody>
                        ...
                        <tr>
                          <td colspan="4" align="center" class="negrita"><table width="699" border="0" cellpadding="0" cellspacing="0">
                            <tr>
                              <th scope="col"><div id="para1" class="negrita"/>Sub Total: <?=$row_compras['subtotal']?></div></th>
                              <th scope="col"><div id="para2" class="negrita"/>IVA: <?=$row_compras['iva']?></div></th>
                              <th scope="col"><div id="para3" class="negrita"/>Total: <?=$row_compras['total']?></div></th>
                              </tr>
                            </table></td>
                        </tr>
                        <tr>
			...
			$nrodetalles++;
				//detalles productos
				$sql2 = "SELECT * FROM productos where id_producto='$field[producto]' "; 
				...
				?>
                 <tr  class="trListaBody" onclick="mClk(this, 'sel_detalles');" id="detalles_<?=$nrodetalles?>" >
                                ...
                                <td width="79" align="center"><?=$field2["codigo"]?><input type="hidden" name="id<?=$nrodetalles?>" class="cell2" style="text-align:center;" value="<?=$field["producto"]?>"/> </td>
                                ...
                                <td  width="131"><input type="text" name="costo<?=$nrodetalles?>" id="costo<?=$nrodetalles?>" class="cell" style="text-align:right; font-weight:bold;" value="<?=$field["precio"]?>" onchange="sumaTotal2(<?=$nrodetalles?>)"  onFocus="numeroFocus(this);" /></td>
                                <td  width="131"><input type="text" name="total<?=$nrodetalles?>" id="total<?=$nrodetalles?>" disabled="disabled"  class="amt" style="text-align:right; font-weight:bold; background-color:transparent; border:none; width:100%;" value="<?=$field["precio"]*$field["cantidad"]?>"  />
                             </tr>
                     ...
                     <input type="hidden" name="veri_prod<?=$nrodetalles?>" value="<?=$field["producto"]?>" />
                        ...
                        <tr>
                          <td colspan="4" align="center"><input type="submit" value="MODIFICAR COMPRAS" /></td>
                        </tr>

...
if ($fBuscar != "") { 
	$filtro.=" where nombre LIKE '%".utf8_decode($fBuscar)."%' OR
...
$query_producto = "SELECT * FROM productos $filtro";
...
<table border="0" cellpadding="0" cellspacing="0" width="760" >
  <tbody>
    ...
    <tr align="center" class="centrar">
      ...
      <td width="314"  valign="baseline" bgcolor="#437bc6"class="negrita">Producto</td>
      <td width="101" valign="baseline" bgcolor="#437bc6" class="negrita">Disponible </td>
      <td width="91" valign="baseline" bgcolor="#437bc6" class="negrita">Vendidos</td>
      <td width="84" valign="baseline" bgcolor="#437bc6" class="negrita">Extraidos</td>
      <td width="144" valign="baseline" bgcolor="#437bc6" class="negrita">Total Comprado</td>
	...
	do {
		//consultamos disponibilidad
							$query_almacen1 = "SELECT sum(cantidad) FROM almacen where id_producto='$row_producto[id_producto]' and (transaccion='COMPRA' or transaccion='COMPRA-MODIFICADA') ";
							$almacen1 = mysql_query($query_almacen1, $conexion) or die(mysql_error());
							$row_almacen1 = mysql_fetch_assoc($almacen1);
							
							$query_almacen2 = "SELECT sum(cantidad) FROM almacen where  id_producto='$row_producto[id_producto]' and transaccion='VENTA' ";
							$almacen2 = mysql_query($query_almacen2, $conexion) or die(mysql_error());
							$row_almacen2 = mysql_fetch_assoc($almacen2);
							
							$query_pedido = "SELECT sum(cantidad) FROM pedido_productos where  id_producto='$row_producto[id_producto]' ";
							$pedido = mysql_query($query_pedido, $conexion) or die(mysql_error());
							$row_pedido = mysql_fetch_assoc($pedido);
							
							$query_almacen3 = "SELECT sum(cantidad) FROM almacen where  id_producto='$row_producto[id_producto]' and transaccion='EXTRAIDO' ";
	...
	?>
      ...
      <tr class="trListaBody">
        ...
        <td align="center"><?php if($row_almacen2["sum(cantidad)"]!="")echo $row_almacen2["sum(cantidad)"];if($row_almacen2["sum(cantidad)"]=="")echo 0; ?></td>
        <td align="center"><?php if($row_almacen3["sum(cantidad)"]!="")echo $row_almacen3["sum(cantidad)"];if($row_almacen3["sum(cantidad)"]=="")echo 0; ?></td>
        <td align="center"><?php if($row_almacen1["sum(cantidad)"]!="")echo $row_almacen1["sum(cantidad)"];if($row_almacen1["sum(cantidad)"]=="")echo 0; ?></td>
        <td background="imagenes/v_back_der.gif">&nbsp;</td>
      </tr>
